...
KP_DISTANCE = 0.002# 0.015
KP_ANGLE = 0.8
STEER_CHANGE_THRESH = 0.75
...
def process(image):
    ...
    image = image[DRIFT:, CUT_LEFT:]  # 裁剪出图片下部
...
def get_control(left_, right_):
    ...
    if is_left_good and is_right_good:
        ...
        if (left_array[:, 1] == right_array[:, 1]).all():
            ...
            distance_error = 0.5 * WIDTH - (a * HEIGHT + b) + DIST_ERR_OFFSET  # 下方交点，横向误差
    ...
    global steer_last
    steer = np.clip(steer, steer_last-STEER_CHANGE_THRESH, steer_last+STEER_CHANGE_THRESH)
